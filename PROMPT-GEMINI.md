...
1. Chỉ import đúng một dòng: `#import "@preview/conic-toan:0.3.9": * ` — TUYỆT ĐỐI không import package nào khác (không cetz, không polylux), không tự định nghĩa lại hàm đã có trong thư viện. Nếu tôi nói file sẽ lưu trong thư mục con (cùng cấp với `lib/`), vẫn dùng đúng dòng import package này.
...
9. Đối với câu tl có nhiều ý hỏi, buộc đặt các ý hỏi trong hàm cot-item.
...
`@preview/conic-toan:0.3.9` là bản mục tiêu của prompt này, và **mọi hàm, mọi
tham số nêu trong tài liệu đều phải hiểu theo API 0.3.9**. Trong lúc bản đang
...
Mới ở 0.3.9: `tieng-anh: true` trong `#de-toan` hoặc `#bai-giang` đổi toàn bộ
nhãn mặc định do lib sinh (`Definition`, `Theorem`, `Note`, `Remark`, `Example`,
`Practice`, `Application`, `Question`, `Solution`, bảng đáp án...) sang English;
nội dung và nhãn truyền tay vẫn giữ nguyên. Mặc định `false` để tương thích.
...
Tôi đưa file `.typ` cũ còn ghi `@preview/conic-toan:0.3.8` hay thấp hơn thì cứ
đổi thẳng dòng import sang `0.3.9`, không phải hỏi — bản mới chỉ THÊM, không bỏ
...
```typst
#import "@preview/conic-toan:0.3.9": *
...
#show: de-toan.with(
  ...
  mon: [MÔN TOÁN 12],
  thoi-gian: "90 phút",
  truong: [SỞ GIÁO DỤC VÀ ĐÀO TẠO \ TRƯỜNG THCS&THPT TÂN THÀNH],
  ma-de: "101",
  gv: "Trần Thị Kim Thu",
...
HÌNH KÈM LỜI GIẢI KHI CÓ NHIỀU MÀN (19/08/2026): trước đây hình `fig-giai:`
chỉ nằm ở màn ĐẦU, các màn "(tiếp)" trắng hình nên đang chiếu phải nhớ lại
hình vừa xem; vì thế người soạn hay chèn hình bằng tay vào từng màn
`#sang-man`. **Nay KHÔNG phải làm thế nữa** — hình tự lặp lại ở mọi màn của
cùng một câu (cả màn tự ngắt lẫn màn `#sang-man`), và phép đo chia màn cũng
đã tính chỗ của hình nên màn "(tiếp)" không bị tràn. ⚠️ **ĐỪNG chèn hình vào
nội dung màn `#sang-man` nữa — sẽ thành HAI hình.** Muốn giữ nếp cũ:
`#hinh-moi-man(false)` cho cả tài liệu, hoặc `fig-giai-moi-man: false` cho
riêng một câu.

CÂU NHIỀU Ý, MỖI Ý MỘT HÌNH: truyền cả MẢNG — `fig-giai: (hinh-y-a, hinh-y-b)`
— hình thứ i dành cho màn thứ i, hết mảng thì giữ hình cuối; đặt `#sang-man` ở
ranh giới a) / b) là mỗi màn hiện đúng hình của ý đang giải. Bản A4 cắt lời
giải theo `#sang-man` rồi ghép từng ý với hình của ý đó thành một khối hai cột
riêng, nên hình ý b) nằm ngang tầm lời giải ý b).

⚠️ **TUYỆT ĐỐI KHÔNG tự dựng `#grid(columns: ...)` để đặt hình bên trong
`loi-giai:`.** Thư viện sẽ không biết là có hình ⇒ hình không lặp lại được ở
các màn sau, mà cả lời giải thành MỘT phần tử nên bộ tự-ngắt-màn cũng không
cắt được: slide tràn thì phần dư bị đẩy sang trang sau còn hình nằm lại trang
trước. Hình kèm lời giải LUÔN đưa ra `fig-giai:`.
...
//   và LẶP LẠI ở mọi màn "(tiếp)" của câu đó (cả màn tự ngắt lẫn màn #sang-man)
//   nên không phải chèn hình tay vào từng màn nữa. Tắt: #hinh-moi-man(false)
//   cho cả tài liệu, hoặc fig-giai-moi-man: false cho riêng một câu.
...
// HĐKP — câu hỏi DẪN NHẬP vào bài mới (thẻ hồng sen, KHÔNG số). Gợi mở,
// KHÔNG buộc có lời giải; khai loi-giai: thì khối ghi "Gợi ý" (không phải
// "Hướng dẫn giải"). Đổi nhãn thẻ bằng nhan:, đổi chữ khối bằng nhan-giai:.
// PHÂN BIỆT: #kham-pha = hoạt động khám phá TRONG bài; #hdkp = MỞ BÀI.
#hdkp([Hoàn thành bảng giá trị của $sin x$, $cos x$, $tan x$, $cot x$ tại
  $x = pi/6$, $x = 0$, $x = -pi/2$.])
#hdkp([Với mỗi số thực $x$, có bao nhiêu điểm $M$ trên đường tròn lượng giác
  sao cho số đo góc lượng giác $(O A, O M)$ bằng $x$?],
  loi-giai: [Đúng một điểm $M$, nên $sin x$ và $cos x$ xác định duy nhất.])
#hdkp([Câu mở đầu ghi thẻ đúng dáng SGK.], nhan: [HĐ1])
```
...
Các hàm `#vd`/`#tn`/`#ds`/`#tln`/`#tl`/`#hd`/`#lt`/`#vdtt`/`#phan` dùng được NGAY sau dòng `#import "@preview/conic-toan:0.3.9": *` — chúng tự nhận biết chế độ hiển thị, KHÔNG cần khai báo `tao-cau-hoi` (dòng `#let (...) = tao-cau-hoi(ho-so)` của file cũ vẫn chạy bình thường). Cú pháp gọi GIỐNG HỆT mục 3 ở mọi chế độ.
...
> **⛔ CẢNH BÁO NGHIÊM TRỌNG — LỖI KHÔNG BIÊN DỊCH ĐƯỢC:** Ở chế độ beamer, TẤT CẢ hàm câu hỏi/hoạt động — `#vd`, `#tn`, `#ds`, `#tln`, `#tl`, `#hd`, `#lt`, `#vdtt`, `#kham-pha`, `#trai-nghiem`, `#thao-luan`, `#hdkp` — bên trong đều tự gọi `slide()` (mà `slide()` dùng `set page(...)`). Do đó:
>
...
> - **Muốn đặt khối hoạt động NGAY BÊN TRONG một `#slide[...]`** (vd làm một bước `#lo(n)` của slide đó) thì dùng **builder nội dòng** `cau-...` — nó chỉ dựng nhãn + thân, KHÔNG tạo slide nên không lỗi: `#cau-hd`, `#cau-lt`, `#cau-vdtt`, `#cau-tl`, và `#cau-kham-pha`, `#cau-trai-nghiem`, `#cau-thao-luan`, `#cau-hdkp` (tham số y hệt bản không có `cau-`, trừ `tieu-de:`).
>
> ```typst
...
ĐÁNH SỐ LẠI — MỖI thể loại có bộ đếm & hàm RIÊNG: `#dat-lai-cau()` chỉ đặt lại nhóm **Câu** (`#tn`/`#ds`/`#tln`/`#tl`); `#dat-lai-cau-vd()` Ví dụ, `#dat-lai-cau-hd()` Hoạt động, `#dat-lai-cau-lt()` Luyện tập, `#dat-lai-cau-vdtt()` Vận dụng thực tế, `#dat-lai-cau-pp()` Phương pháp (khung `#phuong-phap`); `#dat-lai-cau-tat-ca()` đặt lại CẢ 6 nhóm cùng lúc (hành vi cũ). Không tham số hoặc `(0)` → về 1; `#dat-lai-cau(3)` → đánh tiếp từ 4 (tổng quát `(n)` → từ n + 1). Dùng khi sang đề mới / phần mới trong cùng một file.
...
Khung nội dung LÝ THUYẾT (không có lời giải kèm theo, gọi trực tiếp bên trong `#slide[...]` do bạn tự tạo): `#dinh-nghia[...]`, `#dinh-ly[...]`, `#tinh-chat[...]`, `#cong-thuc[...]`, `#chu-y[...]`, `#ghi-nho[...]`, `#nhan-xet[...]` (mục "Nhận xét" của SGK), `#luyen-tap[...]` (khung nêu đề KHÔNG kèm lời giải — luyện tập CÓ lời giải thì dùng `#lt(...)`; hai kiểu chung một bộ đếm nên số vẫn liên tục), `#phuong-phap(ten: [...])[...]`. Hai cột: `#chia-cot(trai, phai)` hoặc `#chia-cot(a, b, ti-le: (3fr, 2fr))`. Các bước: `#buoc(hien-dan: true, [B1...], [B2...])`.

**`#phuong-phap` — nêu PHƯƠNG PHÁP giải một DẠNG toán** (không phải giải một bài cụ thể). Dùng khi mở đầu một dạng bài, ngay trước loạt `#vd(...)` áp dụng dạng đó. Nhãn cam đất, TỰ ĐÁNH SỐ bằng bộ đếm RIÊNG (độc lập với Ví dụ / Luyện tập / Hoạt động):
...
Ra `PHƯƠNG PHÁP 1 — Tìm số hạng tổng quát`. Bỏ `ten:` thì chỉ còn nhãn và số. `#phuong-phap(so: false)[...]` bỏ hẳn số và KHÔNG làm bộ đếm nhảy. Đánh số lại: `#dat-lai-cau-pp()`. Thân khung nên là các gạch đầu dòng `*Bước 1:*`, `*Bước 2:*`… — đó là dáng chuẩn của sách tham khảo. Khung dài tự ngắt sang trang/màn kế tiếp; đặt được ngay trong `#slide[...]` thủ công vì nó CHỈ là nội dung, KHÔNG tự tạo slide như `#hd`/`#lt`.
...
// Mọi BBT và bang-xet-dau tự co đồng đều theo bề rộng vùng chứa. Có thể đặt
// thẳng trong #chia-2-cot mà không cần tự scale/resize; ngoài cột giữ cỡ cũ.
...
// KHÔNG BẮT BUỘC khai xmin/xmax/ymin/ymax: bỏ trống thì gói TỰ DÒ phạm vi
...
//   ĐƯỜNG TRÒN NGOẠI TIẾP RẤT HAY TRÀN RA NGOÀI KHUNG — cách gọn nhất là BỎ
//   TRỐNG cửa sổ để gói tự dò: #hinh(w: 5cm, ctx => { ... })
//   Muốn tự tay đặt khung thì dùng khung-vua:
...
// Đường gióng: giong(P) tự đặt nhãn theo dấu toạ độ để né trục/đường gióng;
//   huong-x/huong-y mặc định auto, chỉ truyền hướng cụ thể khi cần ghi đè.
...
### 5d-bis. Nón, trụ & cầu có NÉT KHUẤT TỰ ĐỘNG, kể cả hai khối che nhau (mat-cong.typ)
...
**MẶT CẦU** — `khoi-cau(tam:, r:, xich-dao: true, kinh-tuyen: 0, nghieng:, huong:, truc:, mau:, to:)`; ở đây `tam` là TÂM cầu, KHÔNG có `cao`.
...
#mat-cong(mat-non(r: 2, cao: 4, mau: red),      // cầu chen vào lòng nón
  khoi-cau(tam: (0, 0, 1.5), r: 1.5, mau: blue))
// vĩ tuyến vẽ thêm — cũng tự chia liền/đứt:
#mat-cong(khoi-cau(r: 2.2), duong: ((pts: tron-ngang((0, 0, 1.2), 1.84),),))
```

⚠️ `nghieng`/`huong`/`truc` của cầu là TRỤC CỰC — chỉ đổi xích đạo và kinh
tuyến, KHÔNG đổi đường bao (đường bao luôn là đường tròn lớn vuông góc hướng
nhìn). Đừng hứa với người dùng rằng nghiêng cầu thì bao đổi theo.

⚠️ Engine coi khối là ĐẶC, KHÔNG trong suốt: "cầu nội tiếp trụ/nón" sẽ ra cầu
gần như TOÀN NÉT ĐỨT vì nằm hẳn trong lòng khối kia. Muốn dáng trong suốt của
SGK thì vẽ cầu ở lời gọi `mat-cong` RIÊNG rồi chồng lên.

**NỘI/NGOẠI TIẾP KHỐI ĐA DIỆN** — ghép `mat-cong` với `da-dien`. Khối nào LỚN
hơn thì để khối đó dựng khung, khối kia vẽ kèm qua `them:` / `truoc:`:

```typ
#let cm = chieu-truc-giao(ngang: 20deg, cao: 20deg)   // MỘT camera cho cả hai
// cầu NỘI tiếp lập phương cạnh 3 (r = 1.5)
#da-dien(..khoi-hop((-1.5, -1.5, -1.5), (3, 0, 0), (0, 3, 0), (0, 0, 3)),
  cam: cm, them: (ctx, p) => ve-mat-cong(ctx, khoi-cau(r: 1.5), cam: cm))
// nón NỘI tiếp chóp tứ giác đều (r = R đáy · cos45)
#da-dien(..khoi-chop-deu(n: 4, R: 2, cao: 3.6),
  cam: cm, them: (ctx, p) => ve-mat-cong(ctx, mat-non(r: 1.414, cao: 3.6), cam: cm))
// trụ NGOẠI tiếp lăng trụ lục giác đều (r = R lăng trụ)
#mat-cong(mat-tru(r: 1.8, cao: 3.4), cam: cm,
  truoc: (ctx, p) => ve-da-dien(ctx, ..khoi-lang-tru-deu(n: 6, R: 1.8, cao: 3.4), cam: cm))
```

⚠️ BẮT BUỘC truyền CÙNG `cam:` cho cả hai — mặc định của chúng KHÁC nhau
(`da-dien` → `chieu-xien()`, `mat-cong` → `chieu-truc-giao(...)`), quên là hai
hình lệch hẳn nhau mà KHÔNG báo lỗi. Lối này KHÔNG có che khuất chéo: mỗi khối
chỉ tự lo nét khuất của chính nó (đúng lối SGK cho bài nội/ngoại tiếp).

**CẦN CHE KHUẤT CHÉO THẬT → `khoi-da-dien`** (một lời gọi, cùng engine bắn
tia). Dùng khi khối đa diện và mặt cong ĐÂM XUYÊN nhau:

```typ
// cầu đâm xuyên lập phương: cạnh hộp sau cầu ĐỨT, biên cầu trong lòng hộp ĐỨT
#mat-cong(
...
nhận thẳng khối dựng sẵn của da-dien qua `..`. Khối đa diện ĐỨNG RIÊNG thì hai
đường đi cho kết quả GIỐNG NHAU, nên bài chỉ có đa diện cứ dùng `da-dien` như
cũ. Khác biệt duy nhất: `to:` ở đây tô BÓNG KHỐI một màu, không tô từng mặt.
...
GIỚI HẠN: mới có nón · trụ · cầu (chưa có mặt xuyến, paraboloid…); ranh giới
liền/đứt rơi đúng một mắt lưới của đường vẽ (`n:` tăng thì mịn hơn). Phép thử
che khuất là GIẢI TÍCH nên không bỏ sót nét khuất. Đường BIÊN cố ý không để
chính khối của nó che, kẻo nét biên chập chờn.
...
// TỨ DIỆN ĐỀU (0.3.5): khoi-tu-dien-deu(a:, pha: 270deg, ten:)
//   pha = góc quay ĐÁY. Mặc định 270°: A trái · C phải · B trước · D đỉnh,
//   cạnh khuất AC. Dáng cũ (trước 0.3.5): pha: 210deg (hoặc pha: auto).
//   ⚠️ ĐỪNG dùng chieu-xien khi k·cos(goc) ≈ 1/√3 ≈ 0.577 (vd goc: 15deg,
//   k: 0.6) — hai đỉnh đáy chồng nhau, một mặt co thành nét, đường vẽ thêm
//   trong đáy đọc không ra. Nên dùng:
#da-dien(..khoi-tu-dien-deu(a: 3.4),
  cam: chieu-truc-giao(ngang: 25deg, cao: 25deg))
// Tứ diện đều lối SGK: trung tuyến đáy AM + đường cao DO xuống trọng tâm
#{
  let td = khoi-tu-dien-deu(a: 3.4)
  let (A, B, C, D) = td.dinh
  let M = trung-diem-3d(B, C)
  let O = tam-3d((A, B, C))
  da-dien(..td, w: 5.4cm, to: teal.lighten(88%),
    cam: chieu-truc-giao(ngang: 25deg, cao: 25deg),
    diem: ((M, $M$), (O, $O$)),
    duong: ((A, M), (D, M), (D, O, (vuong: A))),
  )
}
...
- [ ] File bắt đầu bằng `#import "@preview/conic-toan:0.3.9": *` (đúng số bản, KHÔNG dùng đường dẫn tương đối `"baigiang.typ"`)?
...
- [ ] Câu có `fig-giai:` và nhiều màn: KHÔNG chèn lại hình bằng tay vào nội dung màn `#sang-man` (từ 19/08/2026 hình tự lặp ở mọi màn — chèn tay là thành HAI hình)?
- [ ] `#vd`/`#hd`/`#lt`/`#vdtt` có `tieu-de:` ngắn gọn; `#tn`/`#ds`/`#tln`/`#tl` **KHÔNG** có `tieu-de:` (để tiện trộn đề)?
- [ ] Hoạt động SGK (HĐ1, HĐ2...) dùng `#hd`, luyện tập củng cố dùng `#lt`, bài "Vận dụng" thực tế dùng `#vdtt` (không dùng nhầm `#tl`)? Hoạt động một-lần KHÔNG số của SGK dùng đúng nhãn: `#kham-pha` (Khám phá), `#trai-nghiem` (Trải nghiệm), `#thao-luan` (Thảo luận)? Câu hỏi MỞ ĐẦU dẫn vào bài mới dùng `#hdkp` (gợi mở, khối lời giải ghi "Gợi ý")?
